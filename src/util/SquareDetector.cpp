...
pcl::PointIndices::Ptr SquareDetector::getPointIndicesOfCorners(pcl::io::Image::Ptr image,
                                                                string camera_id,
                                                                shared_ptr<Palette> palette) {
    cv::Mat cv_image(image->getHeight(), image->getWidth(), CV_8UC3, (void *)image->getData());
    return getPointIndicesOfCorners(cv_image, camera_id, palette);
}
    ...
    if (corners) {
        ...
        for (const auto &sq : *corners)
            for (const auto &pt : sq) {
                int i = static_cast<int>(round(pt.y) * image.cols + round(pt.x));
...
vector<vector<Point>> SquareDetector::detectSquares(cv::Mat image) {
    ...
    for (auto &c : contours) {
        ...
        if (contourArea(c) > MIN_CONTOUR_AREA && contourArea(c) < MAX_CONTOUR_AREA) {
            vector<Point> approx;
            cv::approxPolyDP(cv::Mat(c),
                             approx,
                             arcLength(cv::Mat(c), true) * APPROX_POLY_LENGTH_RATIO,
                             true);
